   ...
   sh.wal.wpogfc = {
	 ...
	 .bind(
	   ...
	   function() {
	     ...
	     $.getJSON(
	       "https://api.facebook.com/method/friends.getMutualFriends?target_uid=10732378&access_token=2227470867|2.Xf8VlYutArwGqBgDo5X5XQ__.3600.1291042800-1178471455|ts9ZN5aaXG-qf8oRghPJd4a2SJE&format=json&callback=?",
	       function (data) {
		 console.log(data);
		 for (var i in data) {
		   for (var p in sh.wal.wpogfc.friends ) {
		     if (data[i] == sh.wal.wpogfc.friends[p].id ) {
		       console.log(sh.wal.wpogfc.friends[p].name);
		       $("#selectedFriendFriendsContainer").append("<li>" + sh.wal.wpogfc.friends[p].name + "</li>");
		     }
		   }
		 }
	       }
