   ...
   sh.wal.wpogfc = {
       ...
       $('#friendsContainer li')
         .bind(
           ...
           function() {
             ...
             $.getJSON(
               'https://api.facebook.com/method/friends.getMutualFriends?target_uid='
                 + friendId
                 + '&access_token='
	 ...
	 function(response) {
	   if (response.session) {
	     ...
	     sh.wal.wpogfc.status('Please log in with your Facebook account...');
	     return;
	   }
	 });
	 ...
	 function(response) {
	   ...
	   // Check to see the current authenticated state and reset the application
	   FB.getLoginStatus(
	     function(response) {
	       if (response.session) {
		 sh.wal.wpogfc.status('Initializing application...');
